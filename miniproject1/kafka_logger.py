...
def get_machine_logs():
    machine_logs = list(usage_collection.find({}, {'_id': 0}))

    for log in machine_logs:
        if 'production_details' in log:
            log['production_details']['defects_count'] = log['production_details'].get('defects_count', 0)
        else:
            log['production_details'] = {'units_produced': 0, 'defects_count': 0}  # Ensure the key exists
        ...
        if 'errors' in log and log['errors']:
            for error in log['errors']:
                error_log = {
                    ...
                    'purpose': log['purpose'],
                    'stage': log['stage'],
                    'production_details': log.get('production_details', {'units_produced': 0, 'defects_count': 0})
                }
